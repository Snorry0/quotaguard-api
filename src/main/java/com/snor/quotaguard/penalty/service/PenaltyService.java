...
public class PenaltyService {
    ...
    public PenaltyEvent applyQuotaViolation(User user, UserQuota quota) {
        quota.incrementPenaltyLevel();
        int newLevel = quota.getPenaltyLevel();
    ...
    @Transactional(readOnly = true)
    public void ensureNoBlockingPenalty(User user) {
        findActiveBlockingPenalty(user).ifPresent(activePenalty -> {
    ...
    @Transactional(readOnly = true)
    public List<PenaltyEventResponse> getCurrentUserPenaltyHistory() {
        return penaltyEventRepository.findByUserOrderByStartTimeDesc(currentUserProvider.getCurrentUser()).stream()
